...
print("🛡️ Receiver listening on port 5000 (AES-256-GCM + Anti-Replay Active)...")
...
try:
    while True:
        ...
        # Step 3: Unpack Nonce (12B), Auth Tag (16B), Ciphertext
        nonce = payload[:12]
        auth_tag = payload[12:28]
        ciphertext = payload[28:]

        try:
            # Step 4: Decrypt & Authenticate
            ...
            # Step 5: Anti-Replay Engine Check
            if incoming_seq <= last_seen_sequence:
                ...
                print(
                    f"   [Sensors] Temp: {temp}°C | Press: {press} PSI | RPM: {rpm} | Vib: {vib} mm/s\n"
                )
